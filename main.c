 ...
 *
 *  Next step in course: 5. Colorful single line comments
 *
 *
 */
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (GPAD_TAB_STOP - 1) - (rx % GPAD_TAB_STOP);
    rx++;
  }
  return rx;
}
...
void editorUpdateRow(erow *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs*(GPAD_TAB_STOP - 1) + 1);
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        char welcome[80];
        int welcomelen = snprintf(welcome, sizeof(welcome), "GurkPad editor -- Version %s", GPAD_VERSION);
